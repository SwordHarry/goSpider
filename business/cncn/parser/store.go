...
func (s *StoreParser) Parse(contents []byte, url string) engine.ParseResult {
	return parseStore(contents, url, s.foodName, s.cityName)
}

func (s *StoreParser) Serialize() (name string, args interface{}) {
	return config.ParserStore, *s
}

func NewStoreParser(foodName string, cityName string) *StoreParser {
	return &StoreParser{foodName: foodName, cityName: cityName}
}
